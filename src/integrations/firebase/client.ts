// Firebase client with Firebase AI Logic (Gemini API)
    ...
    invoke: async (functionName: string, options: { body: any }) => {
      try {
        if (functionName === 'student-chat') {
          ...
          const USE_AI = !!FIREBASE_PROJECT_ID && import.meta.env.VITE_USE_REAL_API === 'true';
          
          console.log('=== CHAT MODE ===');
          console.log('Firebase Project ID:', FIREBASE_PROJECT_ID);
          console.log('USE_REAL_API:', import.meta.env.VITE_USE_REAL_API);
          console.log('Will use:', USE_AI ? 'FIREBASE VERTEX AI (GEMINI)' : 'SMART MOCK RESPONSES');
          ...
          // FIREBASE AI LOGIC - Use Gemini for realistic responses
          console.log('=== USING FIREBASE AI LOGIC (GEMINI) ===');
          
          try {
            console.log('=== MAKING FIREBASE AI CALL ===');
            
            // Initialize Firebase app for Vertex AI
            ...
            // Generate system prompt based on profile and gender
            const systemPrompts = {
              male: {
            ...
            const conversationHistory = messages.map(msg => 
              `${msg.role === 'user' ? 'Student vraagt' : 'Jij antwoordt'}: ${msg.content}`
            ).join('\n');
...
Reageer nu als deze student op het laatste bericht. Houd het natuurlijk, vriendelijk en informatief. Maximaal 2-3 zinnen. Gebruik Nederlandse studentenslang waar dat natuurlijk is.`;
            
            console.log('Sending prompt to Firebase Vertex AI...');
            const result = await model.generateContent(prompt);
            const response = await result.response;
            const responseText = response.text();
            
            console.log('Firebase Vertex AI response received:', responseText);
            ...
            // Fallback to smart mock response if AI fails
            console.log('=== FALLING BACK TO SMART MOCK ===');
            const generateSmartResponse = (profile, gender, userMessage) => {
              const lowerMessage = userMessage.toLowerCase();
              ...
              if (lowerMessage.includes('kost') || lowerMessage.includes('geld') || lowerMessage.includes('duur')) {
                return `Qua kosten... ik betaal ${char.cost} per maand voor mijn kamer in ${char.city}. Met DUO krijg ik ongeveer €1100 per maand (studiebeurs + lening). Plus ik werk parttime, dat helpt echt!`;
              }
              
              return `Hey! Leuk dat je interesse hebt in ${char.study}! Ik ben ${char.name} en studeer in ${char.city}. Wat wil je weten?`;
            };
            
            const lastUserMessage = messages[messages.length - 1]?.content || 'Hallo';
            const fallbackResponse = generateSmartResponse(profile, gender, lastUserMessage);
            
            return { 
              data: {
                choices: [{
                  message: {
                    content: fallbackResponse + " (Let op: AI momenteel offline, maar ik help je graag!)"
                  }
                }]
              }, 
              error: null 
